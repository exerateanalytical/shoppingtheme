 ...
 *
 * Renders the REAL WooCommerce cart (the page's wp:woocommerce/cart block) inside
 * the Alluvia chrome. The previous hand-rolled cart used hardcoded prices
 * (ITEM_PRICES = [65,52,68]), a fake "WELCOME10" coupon and client-side totals
 * that never touched WooCommerce. This renders the live, functional cart.
 *
 * @package Shopping
 */
...
/* keep WooCommerce block/classic notices on-brand and legible on the white panel */
...
.alluvia-commerce-inner .wc-block-cart__submit-button,
.alluvia-commerce-inner a.checkout-button,
...
<div class="alluvia-commerce-page">
  <div class="alluvia-commerce-inner">
    ...
    while ( have_posts() ) :
        the_post();
        echo '<h1 class="commerce-page-title">' . esc_html__( 'Your Cart', 'shopping' ) . '</h1>';
        the_content(); // renders the real wp:woocommerce/cart block
    endwhile;
    ?>
  </div>
</div>
